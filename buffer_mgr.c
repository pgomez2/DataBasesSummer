...
{   // Assign the page file, number of pages, and strategy to the buffer pool
    ...
    if (pageFrames == NULL) return RC_MEMORY_ALLOCATION_ERROR;
...
RC unpinPage(BM_BufferPool *const bm, BM_PageHandle *const page) {
    ...
    if (bm == NULL || bm->mgmtData == NULL || page == NULL) {
        return RC_INVALID_INPUT_PARAMS; // Error code for invalid input
    }
    ...
    return pageFound ? RC_OK : RC_PAGE_NOT_FOUND; // Return RC_PAGE_NOT_FOUND if page is not found in the buffer
}
...
RC forcePage(BM_BufferPool *const bm, BM_PageHandle *const page) {
    ...
    if (!bm || !bm->mgmtData || !page) {
        return RC_INVALID_INPUT_PARAMS; // Error code for invalid inputs
    }
   ...
   // Iterate through the buffer pool to find the page
    ...
    return RC_PAGE_NOT_FOUND; // Return error if page not found
}
...
int getNumReadIO(BM_BufferPool *const bm)
{
    // Check if buffer pool or its management data is initialized
    if (bm == NULL || bm->mgmtData == NULL) {
        return RC_BUFFER_POOL_NOT_INITIALIZED ; // Return NULL if buffer pool or its management data is not initialized
    }
